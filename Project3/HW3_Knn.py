...
for q in range(test_bovw.shape[0]):
    dists = np.sum((bow_descs - test_bovw[q]) ** 2, axis=1)
    match_idx, k = find_neighbors(75, dists, classes)
    for num in classnumber:
        if (num in classes[match_idx]) & (num in test_paths[q]):
...
print('Using Knn classification,with k = ', k, ',the successful predictions are ', correct, ' images, for a total of',
      len(test_paths), 'images ,with a ratio of', (success_rate * 100).__round__(2), '%')
for z in range(len(classes)):
    success_rate_byclass[:, z] = class_success[:, z] / len(os.listdir(test_classes[z]))
    print('Successful Prediction Rate,using Knn Classification, for class ', classnumber[z], 'is ',
